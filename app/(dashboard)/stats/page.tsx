...
export default async function StatsPage() {
  ...
  return (
    <div className="p-8">
      ...
      <p className="text-gray-400 text-sm mb-8">Research data overview</p>

      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          <div className="text-2xl font-bold text-red-400">{totalCulled}</div>
          <div className="text-xs text-gray-500 mt-1">Total Culled</div>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          <div className="text-2xl font-bold text-amber-400">{harvestCulled}</div>
          <div className="text-xs text-gray-500 mt-1">Harvested</div>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          <div className="text-2xl font-bold text-white">
            {avgCullWeight ? `${avgCullWeight.toFixed(2)} kg` : "—"}
          </div>
          <div className="text-xs text-gray-500 mt-1">Avg Cull Weight</div>
        </div>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          <h2 className="font-semibold text-white mb-4">Average Weight Over Time</h2>
          <WeightChart data={weightChartData} />
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          <h2 className="font-semibold text-white mb-4">Culls Per Month</h2>
